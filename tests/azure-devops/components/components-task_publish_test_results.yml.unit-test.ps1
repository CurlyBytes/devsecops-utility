...
BeforeAll {
    ...
    $filename = "task_publish_test_results.yml"
...
Describe "task_publish_test_results.yml" {
    ...
    Context "yaml file parameters" {
        It "Has a required parameter <parameter.name> with type <parameter.type>" {
            ...
            $template["parameters"].Where( { $_["name"] -eq $parameter.name } )[0]["type"] | Should -BeExactly $parameter.type
                ...
                @{ parameter = @{ name = "testResultsFormat"; type = "string" }},
                @{ parameter = @{ name = "testResultsFiles"; type = "string" }},
                @{ parameter = @{ name = "searchFolder"; type = "string" }},
                @{ parameter = @{ name = "mergeTestResults"; type = "boolean" }},
                @{ parameter = @{ name = "failTaskOnFailedTests"; type = "boolean" }},
                @{ parameter = @{ name = "testRunTitle"; type = "string" }},
                @{ parameter = @{ name = "buildPlatform"; type = "string" }},
                @{ parameter = @{ name = "buildConfiguration"; type = "string" }}
    ...
    Context "yaml syntax key:value pair validation" {
        ...
        It "Match the expected parameters.yamlFullFilePath" {
            $Elements = 'src/pipeline-as-code/azure-devops/components/task_publish_test_results.yml'
        ...
        It "Match the expected task" {
            $Elements = 'PublishTestResults@2.230.0'
        ...
        It "Match the expected parameters.testResultsFormat" {
            $Elements = '${{ parameters.testResultsFormat }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.testResultsFormat
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }

        It "Match the expected parameters.testResultsFiles" {
            $Elements = '${{ parameters.testResultsFiles }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.testResultsFiles
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }

        It "Match the expected parameters.searchFolder" {
            $Elements = '${{ parameters.searchFolder }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.searchFolder
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }

        It "Match the expected parameters.mergeTestResults" {
            $Elements = '${{ parameters.mergeTestResults }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.mergeTestResults
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }

        It "Match the expected parameters.failTaskOnFailedTests" {
            $Elements = '${{ parameters.failTaskOnFailedTests }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.failTaskOnFailedTests
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }
        
        It "Match the expected parameters.testRunTitle" {
            $Elements = '${{ parameters.testRunTitle }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.testRunTitle
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }
        
        It "Match the expected parameters.buildPlatform" {
            $Elements = '${{ parameters.buildPlatform }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.buildPlatform
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }
        
        It "Match the expected parameters.buildConfiguration" {
            $Elements = '${{ parameters.buildConfiguration }}'
            $templateResources = $template.steps.parameters.objectTask.inputs.buildConfiguration
            $Elements | Should -BeIn $templateResources
            $templateResources | Should -Not -BeNullOrEmpty
        }
    }
