...
variable "AZURE_RESOURCE_TYPE_PREFIX" {
  ...
  validation {
    condition     = contains(["rg", "web", "svplan", "azsqlsv", "sqldb", "api", "stracc", "func"] , var.AZURE_RESOURCE_TYPE_PREFIX)
    error_message = <<EOT
...
variable "SOURCE_NAME" {
    type = string
    description = <<EOT
    (Required) Terraform Source Name registry provider.
...
variable "AZURERM_TERRAFORM_VERSION" {
    type = string
    description = <<EOT
    (Required) Terraform Source Name registry provider.
...
variable "STORAGE_ACCOUNT_PRIMARY_ACCESS_KEY" {
    type = string
    description = <<EOT
    (Required) Storage Account Primary Access Key
